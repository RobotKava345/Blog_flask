...
def get_categories_articles(category_id):
    conn = sqlite3.connect('blog.db')
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute(''' 
...
def index():
    ...
    return render_template("index.html", articles=articles, categories = categories) #Результат, що повертається у браузер
...
def category_page(category_id):
    categories = get_all_categories()
    articles = get_categories_articles(category_id)
    return render_template("index.html", articles=articles, categories = categories)
